...
class gmmReg:
  ...
  to be aligned 
  """
  def __init__(self, cost_fun, target_GMM, source_GMM, optim_iters=50):
  ...
  def register(self, x_init):
    ...
    for i in range(self.optim_iters):
      ...
      args = (self.mu_t, self.phi_t, self.mu_s, self.phi_s, self.sigma)
  ...
  def gauss_transform(self, mu_t, mu_s, phi_t, phi_s, h, Z):
    ...
    for i in range(self.num_clusters):
      for j in range(self.num_clusters):
        ...
        diff = np.subtract(mu_t[i], mu_s[j])
